...
class Dp:
    ...
    def min_coin_combination_top_down(self, coins: List[int], target: int) -> int:
        res = self.top_down_up_dp(coins, target, {})
        return -1 if res == float("inf") else res

    def top_down_up_dp(self, coins: List[int], target: int, memo: Dict[int, int]) -> int:
        if target == 0:
            return 0
        if target in memo:
            return memo[target]
        ...
        for coin in coins:
            if coin <= target:
                min_coins = min(min_coins, 1 + self.top_down_up_dp(coins, target - coin, memo))
    ...
    @staticmethod
    def neighbour_burglary(houses: List[int]) -> int:
        if not houses:
            return 0
        if len(houses) == 1:
            return houses[0]
        ...
        for i in range(2, len(houses)):
            current_profit = max(prev_max_profit, houses[i] + prev_prev_max_profit)
            prev_prev_max_profit = prev_max_profit
            prev_max_profit = current_profit
    ...
    def longest_palindrome_in_a_string(self, s: str) -> str:
        n = len(s)
        start, max_len = 0, 0
